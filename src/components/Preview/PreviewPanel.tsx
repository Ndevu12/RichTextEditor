...
function formatHTML(html: string): string {
  ...
  for (const token of tokens) {
    ...
    } else if (token.startsWith('<') && !token.endsWith('/>') && !token.startsWith('<br') && !token.startsWith('<img') && !token.startsWith('<hr')) {
      formatted += `${'  '.repeat(indent)}${token}\n`;
      indent++;
...
export function PreviewPanel({ mode }: PreviewPanelProps) {
  ...
  return (
    <div className="rte-preview" role="region" aria-label={`${mode === 'html' ? 'HTML' : 'Markdown'} preview`}>
      <div className="rte-preview__header">
        <span className="rte-preview__label">
          {mode === 'html' ? 'HTML' : 'Markdown'}
        </span>
        <CopyButton text={output} />
      </div>
      <pre className="rte-preview__code">
        <code>{output || <span className="rte-preview__empty">Start typing to see output...</span>}</code>
      </pre>
    </div>
  );
}
